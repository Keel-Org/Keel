...
**This repository contains two things:** the language design (below, and in full in
`spec/README.md`), and a working **Stage-0 reference interpreter** — the C implementation
that, per the bootstrap plan in §9, brings the language up so it can begin to be written in
itself. The interpreter is real and runnable; it executes every example in `examples/`,
formats Keel canonically, runs property tests with shrinking, and runs a Keel lexer that is
itself written in Keel. A short, honest account of *what the interpreter realizes today
versus what it defers to a later stage* runs alongside each section and is consolidated under
...
runner), and `fmt` (the one canonical formatter).
...
The full reference, including every spectrum-and-resolution rationale, is in `spec/README.md`.
...
`bootstrap/lexer.keel` is **a lexer for Keel, written in Keel**, running on the C interpreter,
tokenizing Keel source into keywords, identifiers, numbers, strings, and operators. It uses
only language features the interpreter realizes faithfully, which is the point: the subset
needed to describe the language's own front end is already real. This is the first concrete
link in the self-hosting chain — Stage 0 reaching toward Stage 1. A full self-hosting compiler
extends exactly this approach through the parser, the type checker (where the refinement
*solver* would finally appear for real), and a code generator.
...
```
src/keel.c            the entire Stage-0 interpreter (C11, single file)
...
spec/README.md        the full language design specification
DESIGN.md             implementation notes: faithful vs. stand-in, in detail
Makefile              build · examples · test · bootstrap · fmt-check
```
